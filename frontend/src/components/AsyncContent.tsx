...
export const AsyncContent = <T,>({
        ...
        return (
            <div className="flex flex-col items-center justify-center p-8 text-center min-h-[30vh]">
                <div className="bg-yellow-50 text-yellow-800 p-6 rounded-lg max-w-lg border border-yellow-200 shadow-sm">
                    ...
                    <h3 className="text-lg font-bold mb-2">{t('components.asyncContent.noInternet')}</h3>
                    <p>{t('components.asyncContent.checkNetwork')}</p>
                </div>
                <button
                ...
                >
                    {t('components.asyncContent.reload')}
                </button>
            </div>
        );
    }
    ...
    if (isHardLoading && showLoader) {
        return loadingComponent ? (
            ...
            <div className="flex flex-col justify-center items-center p-12 min-h-[50vh]">
                ...
                <span className="text-gray-500 font-medium animate-pulse">{t('components.asyncContent.loadingContent')}</span>
            </div>
        );
    }
    ...
    if (error) {
        // We cannot use getErrorMessage helper easily inside the component if we want hooks,
        // but since we moved getErrorMessage to be hooked or we can pass t to it?
        // Wait, getErrorMessage was defined outside.
        // Let's refactor getErrorMessage to be inside or accept t, OR just inline the logic since we have t here.
        ...
        else message = t('components.asyncContent.unexpectedError');
        ...
        return (
            <div className="flex flex-col items-center justify-center p-8 text-center min-h-[30vh]">
                <div className="bg-red-50 text-red-700 p-6 rounded-lg max-w-2xl border border-red-100 shadow-sm">
                    ...
                    <h3 className="text-lg font-bold mb-2">{t('components.asyncContent.somethingWrong')}</h3>
                    <p>{message}</p>
                </div>
                <button
                    onClick={() => window.location.reload()}
                    className="mt-6 px-4 py-2 border border-transparent text-sm font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 cursor-pointer"
                >
                    {t('components.asyncContent.tryAgain')}
                </button>
            </div>
        );
    }
